...
async def process_delivery_status(message: Message, state: FSMContext):
    ...
    try:
        # Используем rowid вместо id
        orders_data = db.fetchall(
            'SELECT rowid, cid, usr_name, usr_address, products, status FROM orders WHERE cid=?',
            (message.chat.id,)
        )
        ...
        await bot.send_message(message.chat.id, "Произошла ошибка при загрузке ваших заказов.")
        return
    try:
        ...
        product_dict = {}
        await bot.send_message(message.chat.id, "Предупреждение: Не удалось загрузить информацию о товарах. Детали заказа могут быть неполными.")
    if not orders_data:
        await bot.send_message(message.chat.id, 'У вас нет активных заказов.')
    else:
...
async def delivery_status_answer(message: Message, orders_data: list, product_dict: dict):
    for order in orders_data:
        ...
        try:
            ...
            await bot.send_message(message.chat.id, order_info, parse_mode='HTML')
